...
export const useRoomList = () => {
  ...
  // 수동으로 특정 룸 업데이트
  const updateRoom = useCallback((roomId: string, updates: Partial<ChatRoomListItemDto>) => {
    // 스토어 업데이트
    useChatRoomStore.getState().updateChatRoom(roomId, updates);
    
    // Query 캐시도 업데이트
    queryClient.setQueryData(['chatRooms'], (oldData: ChatRoomInfo[] | undefined) => {
  ...
  // 새 메시지 수신 시 처리
  const handleNewMessage = useCallback((roomId: string, content: string, timestamp: string) => {
  ...
  return {
    // 데이터
    chatRooms: getSortedRooms(), // 항상 정렬된 목록 반환
    totalUnreadCount,
...
// 타입 export
export type UseRoomListReturn = ReturnType<typeof useRoomList>;
