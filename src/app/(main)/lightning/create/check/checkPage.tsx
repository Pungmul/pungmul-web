...
export default function LightningCreateCheckPage() {
  ...
  if (
    ...
    alert("모든 필드를 입력해주세요.");
    router.replace("/lightning?create=true");
    return;
  }
  ...
  useEffect(() => {
    const getLocationInfo = async () => {
      ...
      if (response.ok) {
        ...
        console.log(x, y);
        setLocationPoint({ lat: y, lng: x });
        setLocation({ latitude: y, longitude: x });
      }
    };

    getLocationInfo();
  }, [address, setLocationPoint]);
  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      <div id="map" className="flex-grow w-full">
        <div
          className="absolute w-12 h-12 flex items-center justify-center cursor-pointer shadow-lg z-10 rounded-full bottom-4 bg-white right-4"
      ...
      <div className="flex flex-col flex-grow">
        ...
        >
          ...
          <div
            ...
            style={{ height: 24, paddingLeft: 4, paddingRight: 4 }}
          >
            <div style={{ fontSize: 14, color: "#9A9A9A" }}>대상</div>
            <div style={{ fontSize: 14 }}>{target}</div>
          </div>
          <div
            className="flex flex-row justify-between items-center"
            style={{ height: 24, paddingLeft: 4, paddingRight: 4 }}
