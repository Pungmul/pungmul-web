...
export default function ChatList() {
  ...
  return (
    <div className="h-full lg:w-[360px] md:border-r md:border-gray-200 w-full flex flex-col overflow-y-auto z-10">
      <div className="flex flex-col sticky top-0 bg-white z-10">
        ...
        >
          ...
          <div className="flex flex-row" style={{ gap: 8 }}>
            {/* 돋보기로 수정 */}
            <MagnifyingGlassIcon
              className="w-[32px] h-[32px] cursor-pointer p-[4px]"
              onClick={() => setIsSearching(true)}
            />
            {/* 채팅 추가로 변경 */}
            <div
              className="cursor-pointer flex justify-center items-center p-[4px]"
        ...
        {isSearching && (
          <div className="flex flex-row items-center justify-between px-[12px] py-[12px] bg-[#EEEEEE] w-full flex-shrink-0">
            <div className="flex flex-row items-center justify-between w-full bg-white rounded-[8px] px-[4px]">
              <input
                type="text"
                name="search"
                id="search"
                ...
                className="w-full h-full bg-white rounded-[8px] px-[8px] py-[8px] outline-none"
                placeholder="검색"
              />
              <div
                className="flex flex-row items-center justify-center w-[24px] h-[24px] bg-[#EEEEEE] rounded-full mr-[8px] cursor-pointer"
                onClick={() => {
                  setSearchKeyword("");
                  setIsSearching(false);
                }}
              >
                <XMarkIcon className="w-[16px] h-[16px] fill-white" />
              </div>
            </div>
          </div>
        )}
